...
# All credit to:
# https://twitter.com/SteveSimple
# https://twitter.com/DanielLHinton

# Ruby implementation of https://utxo.live/oracle/UTXOracle.py
...
client = BitcoinRPC.new('http://foo:bar@127.0.0.1:8332')
#client = BitcoinRPC.new('http://foo:bar@127.0.0.1:18332')

block_count        = client.getblockcount
block_hash_b       = client.getblockhash(block_count)
blockheader       = client.getblockheader(block_hash_b, true)
...
number_of_bins = output_bell_curve_bins.count
...
# 6
puts ("Reading all blocks on #{price_day_date_utc}...")
puts ("This will take a few minutes (~144 blocks)...")
puts ("Height\tTime(utc)\t Completion %")
...
while target == day do
  ...
  puts ("#{block_height}\t#{time.strftime("%H:%M:%S")}\t#{progress_estimate}")
...
puts "blocks_on_this_day: #{blocks_on_this_day}"
